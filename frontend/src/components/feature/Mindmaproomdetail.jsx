...
const Mindmaproomdetail = ({ data }) => {
  ...
  const processedData = useMemo(() => {
    if (!id || !localData.nodes || !localData.relationships) {
      return { nodes: [], links: [] };
    }

    // 선택된 노드의 루트 노드 찾기
    const findRootNode = (nodeId, visited = new Set()) => {
    ...
    // 선택된 노드와 같은 루트를 가진 모든 노드 찾기
    const rootNodeId = findRootNode(id);
    ...
    if (!rootNodeId) return { nodes: [], links: [] };
    
    findAllConnectedNodes(rootNodeId);

    // 필터링된 노드와 관계 생성
    const filteredNodes = localData.nodes.filter(node => connectedNodes.has(node.id));
    ...
    // 이후 Mindmap.jsx의 processedData 로직과 동일하게 처리
    const calculateDepth = (nodeId, visited = new Set()) => {
      if (visited.has(nodeId)) return 0;
      visited.add(nodeId);

      const relationships = filteredRelationships.filter(rel => rel.source === nodeId);
    ...
    const calculateLevel = (nodeId, parentId = null, level = 0, visited = new Set()) => {
      ...
      const children = filteredRelationships
        .filter(rel => rel.source === nodeId)
        .map(rel => rel.target);
    ...
    return {
      nodes: filteredNodes.map(node => ({
        ...node,
        color: node.id === id ? '#FF6B6B' : 
               node.id === rootNodeId ? '#4299E1' : 
               '#718096'
      })),
